...
class StatisticsDetailScreen extends BaseScreen<StatisticsDetailViewModel> {
  ...
  Widget _buildTaskList() {
    // imageUrl이 있는 항목만 필터링
    final filteredTasks =
        viewModel.taskList.where((task) => task.imageUrl != null).toList();

    return ListView.builder(
        itemCount: filteredTasks.length,
        itemBuilder: (context, index) {
          bool isLastItem = index == filteredTasks.length - 1;
          return _buildTaskDetail(filteredTasks[index], isLastItem);
        });
  }
